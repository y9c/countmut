...
/* Create an evaluator for the given -e / -p expressions and the -o output-row
 * template (any may be NULL).  Returns NULL when there is nothing to evaluate.
 * On a compile error the offending message is printed to stderr and that
 * filter is disabled; use cm_expr_valid() first to turn syntax errors into a
 * hard exit.  `output_tpl` is a row template: literal text plus `{expr}`
 * placeholders evaluated per site in the pile namespace (e.g.
 * "{pos+1}\t{ref}\t{a}/({a}+{t})"). */
cm_expr *cm_expr_new(const char *read_expr, const char *pile_expr,
                     const char *output_tpl);

/* Validate the expressions/template (compile-only).  Returns 1 if all valid
 * or all absent (or NULL -> 1); 0 if a compile error was reported. */
...
/* True if an output-row template (-o) is configured and compiled. */
int cm_expr_has_output(const cm_expr *x);
